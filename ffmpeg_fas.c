...
typedef struct fas_context_struct {
  ...
  struct SwsContext *img_convert_ctx_gray;
...
fas_error_type fas_open_video (fas_context_ref_type *context_ptr, AVFormatContext *pFormatCtx)
{
  ...
  *context_ptr = NULL; // set returned context to NULL in case of error
  ...
  fas_context->format_context         = pFormatCtx;
  ...
  *context_ptr = fas_context;
  ...
  fas_context->img_convert_ctx_rgb = sws_getContext(fas_context->codec_context->width,fas_context->codec_context->height,fas_context->codec_context->pix_fmt,fas_context->codec_context->width,fas_context->codec_context->height,PIX_FMT_BGR24,SWS_BICUBIC, NULL, NULL, NULL);
  fas_context->img_convert_ctx_gray = sws_getContext(fas_context->codec_context->width,fas_context->codec_context->height,fas_context->codec_context->pix_fmt,fas_context->codec_context->width,fas_context->codec_context->height,PIX_FMT_GRAY8,SWS_BICUBIC, NULL, NULL, NULL);

  /*while(fas_context->seek_table.num_entries<=3 && fas_context->is_frame_available)
    fas_step_forward(fas_context);*/
...
fas_error_type fas_close_video (fas_context_ref_type context)
{
  ...
  if (context->img_convert_ctx_rgb)
    sws_freeContext(context->img_convert_ctx_rgb);

  if (context->img_convert_ctx_gray)
    sws_freeContext(context->img_convert_ctx_gray);
	  ...
	  if (frameFinished)
	    {
	      ...
	      if (context->frame_buffer->key_frame || fas_get_frame_index(context) == FIRST_FRAME_INDEX /*|| (!context->seek_table.completed && context->seek_table.num_entries>0 && (fas_get_frame_index(context) - context->seek_table.array[context->seek_table.num_entries-1].display_index>10))*/)
		{
...
/* fas_get_seek_completed */
seek_boolean_type fas_get_seek_completed (fas_context_ref_type context)
{
  return context->seek_table.completed;
}
...
fas_error_type private_seek_to_nearest_key (fas_context_ref_type context, int target_index, int offset)
{
  ...
  if (context->current_dts != seek_entry.last_packet_dts && seek_entry.last_packet_dts>=0)
    {
...
fas_error_type private_convert_to_rgb (fas_context_ref_type ctx)
{
  ...
  if (sws_scale(ctx->img_convert_ctx_rgb,(const uint8_t* const*)(ctx->frame_buffer->data), ctx->frame_buffer->linesize, 0,ctx->codec_context->height,&(ctx->rgb_buffer),ctx->rgb_frame_buffer->linesize))
